...
function linkLyric($record_id, $track, $lyric_id, $band_id)
{
	...
	$q = 'UPDATE tblTracks SET lyricId='.$lyric_id.',bandId='.$band_id.' WHERE recordId='.$record_id.' AND trackNumber='.$track;
	$db->update($q);
	return true;
}
...
function updateLyric($lyric_id, $lyric_name, $lyric_text)
{
	...
	$q = 'UPDATE tblLyrics SET lyricName="'.$lyric_name.'", lyricText="'.$lyric_text.'" WHERE lyricId='.$lyric_id;
	$db->update($q);
	return true;
}
...
function removeLyric($lyric_id)
{
	...
	$db->delete('DELETE FROM tblLyrics WHERE lyricId='.$lyric_id);
	$db->update('UPDATE tblTracks SET lyricId=0,bandId=0 WHERE lyricId='.$lyric_id);
}
...
function addLyric($band_id, $record_id, $track, $lyric_name, $lyric_text = '')
{
	...
	if ($record_id) {
		$db->update('UPDATE tblTracks SET lyricId='.$lyric_id.',bandId='.$band_id.' WHERE recordId='.$record_id.' AND trackNumber='.$track);
	}
	return $lyric_id;
}
