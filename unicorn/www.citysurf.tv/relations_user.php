	...
	foreach($wait as $row) {
		...
		echo '
			<tr>
				...
				<td class="spac pdg rgt">'.nicedate($row['sent_date']).'</td>
				<td class="spac rgt pdg_tt"><a href="user_relations.php?id='.$user->id.'&amp;chg='.$row['id_id'].'#w'.$row['main_id'].'"><img src="'.$config['web_root'].'_gfx/icon_change.gif" alt="" title="Ändra" style="margin-bottom: -4px;" /></a> - <a href="user_relations.php?id='.$id.'&d='.$row['id_id'].'" onclick="return confirm(\'Säker ?\');"><img src="'.$config['web_root'].'_gfx/icon_no.gif" alt="" title="Sluta vänta" style="margin-bottom: -4px;" /></a></td>
			</tr>';
		if ($change_id == $row['id_id']) {
?>
			<tr>
				<td colspan="4" class="pdg">
					<form action="<?=$_SERVER['PHP_SELF'].'?id='.$id.'&chg='.$row['id_id']?>" method="post">
						<select name="ins_rel" class="txt" style="width: 205px; margin-right: 10px;">
<?
						foreach ($rel as $val) {
