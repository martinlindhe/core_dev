...
function addRevision($fieldType, $fieldId, $fieldText, $timestamp, $creatorId, $categoryId = 0)
{
    ...
    $q =
    'INSERT INTO tblRevisions'.
    ' SET fieldId='.$fieldId.',fieldType='.$fieldType.',fieldText="'.$db->escape($fieldText).'",createdBy='.$creatorId.',timeCreated="'.$timestamp.'",categoryId='.$categoryId;
    return $db->insert($q);
}
...
function showRevisions($articleType, $articleId, $articleName)
{
    ...
    $q =
    'SELECT COUNT(*) FROM tblRevisions'.
    ' WHERE fieldId='.$articleId.' AND fieldType='.$articleType;
    ...
    foreach ($list as $row) {
        ...
        case REV_CAT_TEXT_CHANGED:
            echo '<a href="#" onclick="return toggle_el(\'layer_history'.$row['indexId'].'\')">';
