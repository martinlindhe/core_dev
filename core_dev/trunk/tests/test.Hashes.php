...
function test_hashes($hashes, $s)
{
    foreach ($hashes as $hash => $val)
    {
        ...
        if ($val != $chk)
            echo "FAIL ".$hash." of string ".$s." should be ".$chk."\n";
    }

}

test_hashes($hashes_empty, '');
test_hashes($hashes_fox, 'The quick brown fox jumps over the lazy dog');
