...
// The mandatory pre-setup function.  Our name is "ISXEQTrek", and the class is ISXEQTrek.
...
// our instanced class.
ISXPreSetup("ISXEQTrek",ISXEQTrek);
...
bool ISXEQTrek::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQTrek Loaded");
	return true;
}

// shutdown sequence
void ISXEQTrek::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQTrekXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQTrekXML);
...
void ISXEQTrek::ConnectServices()
{
...
void ISXEQTrek::RegisterCommands()
{
	// add any commands
...
void ISXEQTrek::RegisterAliases()
{
	// add any aliases
}

void ISXEQTrek::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQTrek::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQTrek::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQTrek::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQTrek::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQTrek::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQTrekService)
//		pISInterface->ShutdownService(this,hISXEQTrekService);
}

void ISXEQTrek::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQTrekXML
	ISXEQTrekXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQTrekXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQTrekXML,"General"))
	{
