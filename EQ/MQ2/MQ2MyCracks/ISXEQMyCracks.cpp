...
#include "ISXEQMyCracks.h"

// The mandatory pre-setup function.  Our name is "ISXEQMyCracks", and the class is ISXEQMyCracks.
...
// our instanced class.
ISXPreSetup("ISXEQMyCracks",ISXEQMyCracks);
...
unsigned int ISXEQMyCracksXML=0;
...
bool ISXEQMyCracks::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQMyCracks Loaded");
	return true;
}

// shutdown sequence
void ISXEQMyCracks::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQMyCracksXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQMyCracksXML);
...
void ISXEQMyCracks::ConnectServices()
{
...
void ISXEQMyCracks::RegisterCommands()
{
	// add any commands
...
void ISXEQMyCracks::RegisterAliases()
{
	// add any aliases
}

void ISXEQMyCracks::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQMyCracks::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQMyCracks::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQMyCracks::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQMyCracks::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQMyCracks::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQMyCracksService)
//		pISInterface->ShutdownService(this,hISXEQMyCracksService);
}

void ISXEQMyCracks::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQMyCracksXML
	ISXEQMyCracksXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQMyCracksXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQMyCracksXML,"General"))
	{
