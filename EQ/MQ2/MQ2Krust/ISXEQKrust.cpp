...
#include "ISXEQKrust.h"

// The mandatory pre-setup function.  Our name is "ISXEQKrust", and the class is ISXEQKrust.
...
// our instanced class.
ISXPreSetup("ISXEQKrust",ISXEQKrust);
...
bool ISXEQKrust::Initialize(ISInterface *p_ISInterface)
{	pISInterface=p_ISInterface;
	...
	WriteChatf("ISXEQKrust Loaded");
	return true;
}

// shutdown sequence
void ISXEQKrust::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQKrustXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQKrustXML);
...
void ISXEQKrust::ConnectServices()
{
...
void ISXEQKrust::RegisterCommands()
{
	// add any commands
...
void ISXEQKrust::RegisterAliases()
{
	// add any aliases
}

void ISXEQKrust::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQKrust::UnRegisterCommands()
{
	// remove commands
//	pISInterface->RemoveCommand("MyCommand");
}
void ISXEQKrust::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQKrust::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQKrust::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
//	pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQKrust::UnRegisterServices()
{
	// shutdown our own services
//	if (hISXEQKrustService)
//		pISInterface->ShutdownService(this,hISXEQKrustService);
}

void ISXEQKrust::LoadSettings()
{
	bool bChanged=false;

	// open the settings file, and store the ID number in ISXEQKrustXML
	ISXEQKrustXML=pISInterface->OpenSettings(XMLFileName);
...
Any set can be referred to by its ID.  The base set is stored in ISXEQKrustXML.  Note that the settings
...
*/
	...
	if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQKrustXML,"General"))
	{
