<?
	...
	*/
	...
	function gbCountMsgByUserId($user_id)
	{
		global $db;
		if (!is_numeric($user_id)) return false;
		return $db->getOneItem('SELECT COUNT(*) FROM s_usergb gb WHERE gb.user_id = '.$user_id.' AND gb.status_id = "1"');
	}
	...
	{
		global $db, $l;
		return $db->getOneItem('SELECT COUNT(*) FROM s_usergb gb WHERE gb.user_id = '.$l['id_id'].' AND gb.user_read = "0"');
	}
	...
	function gbWrite($msg, $user_id, $is_answer = 0, $private = 0)
	{
		...
		$q = 'INSERT INTO s_usergb SET user_id = '.$user_id.', sender_id = '.$l['id_id'].','.
					'private_id = '.$private.', status_id = "1", user_read = "0",'.
					'sent_cmt = "'.$db->escape($msg).'", sent_html = "0", sent_date = NOW()';

		$insert_id = $db->insert($q);
		$or = array($user_id, $l['id_id']);
		sort($or);
		$db->insert('INSERT INTO s_usergbhistory SET users_id = "'.implode('-', $or).'", msg_id = '.$insert_id);
		if($is_answer) {
			$db->update('UPDATE s_usergb SET is_answered = "1" WHERE main_id = '.$is_answer.' AND sender_id = '.$user_id.' AND user_id = '.$l['id_id'].' LIMIT 1');
		}
	...
	function gbDelete($msg_id) {
		global $db, $l, $user;
		if (!is_numeric($msg_id)) return false;
		$res = $db->getOneItem('SELECT main_id, status_id, user_id, sender_id, user_read FROM s_usergb WHERE main_id = '.$msg_id.' LIMIT 1');
		if (!empty($res) && count($res) && $res['status_id'] == '1') {
			if($res['user_id'] == $l['id_id'] || $res[3] == $l['id_id']) {
				$db->update('UPDATE s_usergb SET status_id = "2", deleted_id = '.$l['id_id'].', deleted_date = NOW() WHERE main_id = '.$res['main_id'].' LIMIT 1');
	...
	function gbMarkUnread() {
		...
		if(!$user->getinfo($l['id_id'], 'always_unread')) {
			$db->update('UPDATE s_usergb SET user_read = "1" WHERE user_id = "'.$l['id_id'].'" AND user_read = "0"');
	...
	function gbMarkAsRead($_id)
	{
		...
		$db->update('UPDATE s_usergb SET user_read = "1" WHERE user_id = '.$l['id_id'].' AND main_id = '.$_id);
	}
