<?
	...
	*/
	...
	function sendRelationRequest($_id, $_relation_type)
	{
		...
		$c = $db->getOneItem('SELECT main_id FROM s_userrelation WHERE user_id = '.$l['id_id'].' AND friend_id = '.$_id.' LIMIT 1');
		if ($c) {
			### ÄNDRA!
			$db->update('UPDATE s_userrelquest SET status_id = "2" WHERE user_id = '.$l['id_id'].' AND sender_id = '.$_id);
			$db->update('UPDATE s_userrelquest SET status_id = "2" WHERE user_id = '.$_id.' AND sender_id = '.$l['id_id']);
			$db->delete('DELETE FROM s_userrelation WHERE user_id = '.$l['id_id'].' AND friend_id = '.$_id);
			$db->delete('DELETE FROM s_userrelation WHERE user_id = '.$_id.' AND friend_id = '.$l['id_id']);

			$q = 'INSERT s_userrelquest SET sent_cmt = "'.$db->escape($r).'",'.
					'status_id = "0", sent_date = NOW(), user_id = '.$_id.','.
					'sender_id = '.$l['id_id'];
			...
			$c = $db->getOneItem('SELECT COUNT(*) FROM s_userrelquest WHERE user_id = '.$l['id_id'].' AND sender_id = '.$_id.' AND status_id = "0"');
			if($c > 0) return false; //popupACT('Du har redan blivit tillfrågad.');

			$c = $db->getOneItem('SELECT COUNT(*) FROM s_userrelquest WHERE user_id = '.$_id.' AND sender_id = '.$l['id_id'].' AND status_id = "0"');
			if($c > 0) {
				$q = 'UPDATE s_userrelquest SET ent_cmt = "'.$db->escape($r).'",'.
					'status_id = "0", sent_date = NOW() '.
				...
				$q = 'INSERT INTO s_userrelquest SET user_id = '.$_id.', sender_id = '.$l['id_id'].','.
					'sent_cmt = "'.$db->escape($r).'", status_id = "0", deleted_id = 0, sent_date = NOW()';
	...
	function acceptRelationRequest($_id)
	{
		...
		$q = 'INSERT INTO s_userrelation SET user_id = '.$l['id_id'].','.
				'friend_id = '.$c[0]['sender_id'].', rel_id = "'.$db->escape($c[0]['sent_cmt']).'",'.
				'activated_date = NOW()';
		$db->insert($q);

		$q = 'INSERT INTO s_userrelation SET user_id = '.$c[0]['sender_id'].','.
				'friend_id = '.$l['id_id'].',	rel_id = "'.$db->escape($c[0]['sent_cmt']).'",'.
				'activated_date = NOW()';
		$db->iinsert($q);

		$check = $db->update('UPDATE s_userrelquest SET status_id = "1" WHERE user_id = '.$l['id_id'].' AND sender_id = '.$c[0]['sender_id'].' AND status_id = "0" LIMIT 1');
	...
	function removeRelation($_user_id)
	{
		...
		$db->update('UPDATE s_userrelquest SET status_id = "D" WHERE user_id = '.$l['id_id'].' AND sender_id = '.$_user_id);
		$db->update('UPDATE s_userrelquest SET status_id = "D" WHERE user_id = '.$_user_id.' AND sender_id = '.$l['id_id']);
		$db->delete('DELETE FROM s_userrelation WHERE user_id = '.$l['id_id'].' AND friend_id = '.$_user_id);
		$db->delete('DELETE FROM s_userrelation WHERE user_id = '.$_user_id.' AND friend_id = '.$l['id_id']);
		...
		if ($isFriends) {
			...
			$c = $db->getOneItem($q);
			if ($c) {
				$db->update('UPDATE s_userrelquest SET status_id = "2" WHERE user_id = '.$l['id_id'].' AND sender_id = '.$_user_id.' AND main_id = '.$c);
				#if(mysql_affected_rows()) sysMSG($s['id_id'], 'Relation', 'Your relation with '.$s->alias.' is denied!');
				$user->notifyDecrease('rel', $l['id_id']);
			} else {
				$c = $db->getOneItem('SELECT main_id FROM s_userrelquest WHERE user_id = '.$_user_id.' AND sender_id = '.$l['id_id'].' AND status_id = "0" LIMIT 1');
				if (!empty($c) && count($c)) {
					$db->update('UPDATE s_userrelquest SET status_id = "2" WHERE user_id = '.$_user_id.' AND sender_id = '.$l['id_id'].' AND main_id = '.$c);
	...
	function blockRelation($_id)
	{
		...
		$db->insert('INSERT INTO s_userblock SET rel_id = "u", user_id = '.$l['id_id'].', friend_id = '.$_id.', activated_date = NOW()');
		$db->insert('INSERT INTO s_userblock SET rel_id = "f", user_id = '.$_id.', friend_id = '.$l['id_id'].', activated_date = NOW()');
		return true;
	}
	...
	function unblockRelation($_id)
	{
		...
		$db->delete('DELETE FROM s_userblock WHERE user_id = '.$l['id_id'].' AND friend_id = '.$_id.' AND rel_id = "u" LIMIT 1');
		$db->delete('DELETE FROM s_userblock WHERE friend_id = '.$l['id_id'].' AND user_id = '.$_id.' AND rel_id = "f" LIMIT 1');
		return true;
	}
	...
	function getRelationsCount($_id)
	{
		...
		return $db->getOneItem('SELECT COUNT(*) FROM s_userrelation rel INNER JOIN s_user u ON u.id_id = rel.friend_id AND u.status_id = "1" WHERE rel.user_id = '.$_id);
	}
	...
	{
		global $db, $l;
		return $db->getArray('SELECT q.main_id, q.sent_cmt, q.sent_date, u.id_id, u.u_alias, u.account_date, u.u_picid, u.u_picd, u.status_id, u.lastonl_date, u.u_sex, u.u_birth, u.level_id FROM s_userrelquest q INNER JOIN s_user u ON u.id_id = q.sender_id AND u.status_id = "1" WHERE q.user_id = '.$l['id_id'].' AND q.status_id = "0" ORDER BY q.main_id DESC');
	}
