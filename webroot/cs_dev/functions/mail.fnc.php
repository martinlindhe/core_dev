<?
	...
	*/
	...
	function mailDelete($_id)
	{
		...
		$res = $db->getOneRow('SELECT main_id, status_id, user_id, sender_id, user_read, sender_status FROM s_usermail WHERE main_id = '.$_id.' LIMIT 1');
		if (!empty($res) && count($res) && ($res['status_id'] == '1' || $res['sender_status'] == '1')) {
			if ($res['user_id'] == $l['id_id'] || $res['sender_id'] == $l['id_id']) {
				if ($res['user_id'] == $l['id_id'] && $res['status_id'] == '1') {
				...
				if ($res['sender_id'] == $l['id_id'] && $res['sender_status'] == '1') {
	...
	{
		global $db, $l;
		return $db->getOneItem('SELECT COUNT(*) FROM s_usermail WHERE user_id = '.$l['id_id'].' AND status_id = "1"');
	}
	
	function mailOutboxCount()
	{
		global $db, $l;
		return $db->getOneItem('SELECT COUNT(*) FROM s_usermail WHERE sender_id = '.$l['id_id'].' AND sender_status = "1"');
	}
	...
	function mailInboxContent($_start = 0, $_end = 0)
	{
		...
		$q = 'SELECT m.*, u.* FROM s_usermail m LEFT JOIN s_user u ON u.id_id = m.sender_id AND u.status_id = "1" WHERE m.user_id = '.$l['id_id'].' AND m.status_id = "1" ORDER BY m.sent_date DESC';
	...
	function mailOutboxContent($_start = 0, $_end = 0)
	{
		...
		$q = 'SELECT m.*, u.* FROM s_usermail m LEFT JOIN s_user u ON u.id_id = m.user_id AND u.status_id = "1" WHERE m.sender_id = '.$l['id_id'].' AND m.sender_status = "1" ORDER BY m.sent_date DESC';
		if ($_start || $_end) $q .= ' LIMIT '.$_start.','.$_end;
	...
	{
		global $db, $user, $isAdmin, $l, $s;
		if (!is_array($_arr) || !count($_arr)) return false;
		
		foreach ($_arr as $val) {
			if (!is_numeric($val)) die('fyy');	//invalid data
			$res = $db->getOneRow('SELECT main_id, status_id, user_id, sender_id, user_read, sender_status FROM s_usermail WHERE main_id = '.$val.' LIMIT 1');
			if (!empty($res) && count($res) && ($res['status_id'] == '1' || $res['sender_status'] == '1')) {
				if ($isAdmin || $res['user_id'] == $l['id_id'] || $res['sender_id'] == $l['id_id']) {
					if($res['user_id'] == $l['id_id']) {

						if($res['status_id'] == '1') $user->counterDecrease('mail', $l['id_id']);

						$db->update('UPDATE s_usermail SET status_id = "2" WHERE main_id = '.$res['main_id'].' LIMIT 1');
					} elseif($res['sender_id'] == $l['id_id']) {
						$db->update('UPDATE s_usermail SET sender_status = "2" WHERE main_id = '.$res['main_id'].' LIMIT 1');
					} else {
					...
					if (!$res['user_read']) $user->notifyDecrease('mail', $s['id_id']);
				}
			}
		}		
	...
	{
		global $db, $l;
		return $db->getArray('SELECT rel.main_id, rel.user_id, rel.rel_id, u.id_id, u.u_alias, u.u_picvalid, u.u_picid, u.u_picd, u.status_id, u.lastonl_date, u.u_sex, u.u_birth FROM s_userrelation rel RIGHT JOIN s_user u ON u.id_id = rel.friend_id AND u.status_id = "1" WHERE rel.user_id = '.$l['id_id'].' ORDER BY u.u_alias ASC');
	}

	function sendMail($_to_name, $_cc_name, $_title, $_text, $allowed_html = '', $is_answer = false)
	{
		global $db, $user, $l, $user;
		...
		/*if(!$error && !$isAdmin) {
			$isBlocked = $db->getOneItem("SELECT rel_id FROM s_block WHERE user_id = ".$l['id_id']." AND friend_id = ".$ins_to." LIMIT 1");
		...
		if (!$error && !empty($_cc_name)) {
			...
			if($ins_cc && $ins_cc != $l['id_id'] && $ins_cc != $ins_to) {
				if (!$user->blocked($ins_cc, 3)) {
					$q = 'INSERT INTO s_usermail SET user_id = '.$ins_cc.', sender_id = '.$l['id_id'].','.
								'status_id = "1", sender_status = "1", user_read = "0",'.
								'sent_cmt = "'.$db->escape($_text).'", sent_ttl = "'.$db->escape($_title).'",'.
								'sent_date = NOW()';
					$res = $db->insert($q);
		...
		$q = 'INSERT INTO s_usermail SET user_id = '.$ins_to.', sender_id = '.$l['id_id'].','.
					'status_id = "1", sender_status = "1", user_read = "0", sent_cmt = "'.$db->escape($_text).'",'.
