...
function gettxt($id, $opt = '0', $ext = false) {
	global $db;
	$result = $db->getOneItem('SELECT text_cmt FROM s_text WHERE main_id = "'.$id.'" AND option_id = "'.$opt.'" LIMIT 1');
...
function dopaging($paging, $url, $anchor = '', $width = 'med', $text = '&nbsp;', $vice = 1) {
	...
	if($width == 'med' || $width == 'medmin' || $width == 'big' || $width == 'medbig' || $width == 'biggest') echo '<table cellspacing="0" summary="" style="font-family: Verdana; font-size: 10px; width: '.($width == 'medmin'?'596':($width == 'big'?'596':($width == 'medbig'?'596':($width == 'biggest'?'786':'596')))).'px;"><tr>
