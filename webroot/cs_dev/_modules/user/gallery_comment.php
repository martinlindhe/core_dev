<?
	if(!empty($key)) {
		$res = $sql->queryLine("SELECT main_id, status_id, user_id, pht_date, hidden_id, pht_cmt FROM s_userphoto WHERE main_id = '".secureINS($key)."' LIMIT 1", 1);
   ...
   reloadACT(l('user', 'gallery', $s['id_id'], $res['main_id']));
	...
	if(!empty($_POST['ins_cmt'])) {
		if($l['status_id'] == '1') {
			...
			$r = $sql->queryInsert("INSERT INTO s_userphotocmt SET
			user_id = '".$s['id_id']."',
			id_id = '".$l['id_id']."',
			...
			c_date = NOW()");
			$sql->queryResult("UPDATE s_userphoto SET pht_cmts = pht_cmts + 1 WHERE main_id = '".$res['main_id']."' LIMIT 1");
			#if(!$own) $user->spy($s['id_id'], $res['main_id'], 'BCT', array($l['u_alias'], $s['id_id'], $res['main_id'], $res['blog_title']));
			#elseif($a) {
			#	$id = $sql->queryResult("SELECT b.id_id FROM s_userphotocmt b INNER JOIN s_user u ON u.id_id = b.id_id AND u.status_id = '1' WHERE b.main_id = '".secureINS($a)."' AND b.user_id = '".$l['id_id']."' AND b.status_id = '1' LIMIT 1");
			#	if($id) {
