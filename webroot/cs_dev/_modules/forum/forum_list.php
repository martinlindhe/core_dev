<?
	...
	$r = $db->getOneRow('SELECT main_id, main_ttl, main_cmt, subjects FROM s_ftopic f WHERE f.status_id = "1" AND f.main_id = '.@secureINS($_GET['id']).' LIMIT 1');
	if(empty($r) || !count($r)) {
		errorACT('Rubriken existerar inte.', l('forum', 'start'));
	}
	...
	$c = $sql->queryResult("SELECT COUNT(*) as count FROM s_f WHERE topic_id = '".secureINS($r['main_id'])."' AND parent_id = '0' AND status_id = '1'");
	$d = $sql->queryResult("SELECT COUNT(*) as count FROM s_f WHERE topic_id = '".secureINS($r['main_id'])."' AND parent_id != '0' AND status_id = '1'");

	$paging = paging(@$_GET['p'], 20);

	if($isAdmin)
		$res = $sql->query("SELECT f.main_id, f.topic_id, f.change_date, f.parent_id, f.subject_id, f.status_id, f.sender_id, f.sent_ttl, f.sent_date, CONCAT(SUBSTRING(f.sent_cmt, 1, 50), '...') as sent_cmt, u.id_id, u.u_alias, u.account_date, u.u_sex, u.u_birth, u.level_id, COUNT(t.main_id) - 1 as count FROM s_f f LEFT JOIN s_user u ON u.id_id = f.sender_id AND u.status_id = '1' LEFT JOIN s_f t ON t.topic_id = f.topic_id AND t.top_id = f.main_id AND t.status_id = '1' WHERE f.parent_id = '0' AND f.topic_id = '".$r['main_id']."' AND f.status_id = '1' GROUP BY f.main_id ORDER BY f.change_date DESC LIMIT {$paging['slimit']}, {$paging['limit']}", 0, 1);
	else
		$res = $sql->query("SELECT f.main_id, f.topic_id, f.change_date, f.parent_id, f.subject_id, f.status_id, f.sender_id, f.sent_ttl, f.sent_date, CONCAT(SUBSTRING(f.sent_cmt, 1, 50), '...') as sent_cmt, u.id_id, u.u_alias, u.account_date, u.u_sex, u.u_birth, u.level_id, COUNT(t.main_id) - 1 as count FROM s_f f LEFT JOIN s_user u ON u.id_id = f.sender_id AND u.status_id = '1' LEFT JOIN s_f t ON t.topic_id = f.topic_id AND t.top_id = f.main_id AND t.status_id = '1' WHERE f.parent_id = '0' AND f.topic_id = '".$r['main_id']."' AND f.status_id = '1' GROUP BY f.main_id ORDER BY f.change_date DESC LIMIT {$paging['slimit']}, {$paging['limit']}", 0, 1);

	$own = $sql->query("SELECT o.main_id, o.top_id, o.sent_ttl, o.sent_date FROM s_f o INNER JOIN s_f p ON p.main_id = o.top_id AND p.status_id = '1' WHERE o.sender_id = '".secureINS($l['id_id'])."' AND o.status_id = '1' AND o.view_id = '1' ORDER BY o.main_id DESC LIMIT 5");
	$page = 'list';
