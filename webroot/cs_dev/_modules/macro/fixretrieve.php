<?
	...
	foreach($users as $us) {
		...
		$id = $user->setinfo($us['id_id'], 'gb_count', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'gb_offset', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'chat_count', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'chat_offset', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'gal_offset', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'blog_offset', intval($gb));
		...
		$rel_c = $db->getOneItem('SELECT COUNT(*) FROM s_userrelquest a INNER JOIN s_user u ON u.id_id = a.sender_id AND u.status_id = "1" WHERE a.user_id = '.secureINS($us['id_id']).' AND a.status_id = "0"');
		$id = $user->setinfo($us['id_id'], 'rel_count', intval($rel_c));
		...
		$rel_c = $db->getOneItem('SELECT COUNT(*) FROM s_userrelation INNER JOIN s_user u on u.id_id = friend_id and u.status_id = "1" WHERE user_id = '.secureINS($us['id_id']).'"');
		$id = $user->setinfo($us['id_id'], 'rel_offset', intval($rel_c));
		//check if has got container
		$gotrel = $db->getOneItem('SELECT COUNT(*) FROM s_obj_rel WHERE content_type = "user_head" AND owner_id = '.$us['id_id'].' AND object_id = '.$id[1].' LIMIT 1');
		if(!$gotrel) {
			$user->setrel($id[1], 'user_head', $us['id_id']);
		...
		$id = $user->setinfo($us['id_id'], 'mail_count', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'mail_offset', intval($gb));
		...
		$id = $user->setinfo($us['id_id'], 'spy_count', intval($rel_c));
