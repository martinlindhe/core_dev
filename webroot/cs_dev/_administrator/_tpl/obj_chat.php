<?
	...
	if(!empty($_GET['del']) && is_numeric($_GET['del'])) {
		$res = $sql->queryLine("SELECT user_read, user_id FROM {$t}userchat WHERE main_id = '".$_GET['del']."' LIMIT 1");
		if(!empty($res) && count($res)) {
			$sql->queryUpdate("DELETE FROM {$t}userchat WHERE main_id = '".$_GET['del']."' LIMIT 1");
		}
