<?
	...
	if(!empty($_GET['del']) && is_numeric($_GET['del'])) {
		$res = $sql->queryLine("SELECT user_read, user_id FROM {$tab['user']}mail WHERE main_id = '".$_GET['del']."' LIMIT 1");
		if(!empty($res) && count($res)) {
			if(!$res[0]) $sql->queryUpdate("UPDATE {$tab['obj']} SET content = content - 1 WHERE content_type = '".$thisid."_count' AND owner_id = '".$res[1]."' LIMIT 1");
			$sql->queryUpdate("UPDATE {$tab['user']}mail SET status_id = '2', sender_status = '2' WHERE main_id = '".$_GET['del']."' LIMIT 1");
		}
	...
	$cmt_arr = $sql->queryResult("SELECT COUNT(*) as count FROM {$tab['user']}mail");

	$paging = paging(@$_GET['p'], 20);
	$list = $sql->query("SELECT a.*, u.id_id, u.u_alias, u.u_picd, u.u_picvalid, u.u_picid, u2.id_id AS id_id2, u2.u_alias as u_alias2, u2.u_picd as u_picd2, u2.u_picvalid AS u_picvalid2, u2.u_picid as u_picid2 FROM {$tab['user']}mail a LEFT JOIN {$tab['user']} u ON u.id_id = a.sender_id AND u.status_id = '1' LEFT JOIN {$tab['user']} u2 ON u2.id_id = a.user_id AND u2.status_id = '1' ORDER BY a.main_id DESC LIMIT {$paging['slimit']}, {$paging['limit']}", 0, 1);
...
?>
 ...
 - <em>inl&auml;gg skrivet <?=niceDate($r['sent_date'])?></em> (#<?=$r['main_id']?>)</span><?=(!$r['user_read'])?' <b>[oläst]</b>':'';?></td></tr>
				<tr>
					<td style="width: 100%;"><?='<table cellspacing="0"><tr><td>'.$user->getphoto($r['id_id'].$r['u_picid'].$r['u_picd'], $r['u_picvalid'], 1, 1).'</td><td style="padding-left: 5px;"><div style="width: 470px; overflow: hidden;">'.formatText($r['sent_cmt']).'</div></td></tr></table>';?></td>
					<td><?=$user->getphoto($r['id_id2'].$r['u_picid2'].$r['u_picd2'], $r['u_picvalid2'], 1, 1)?></td>
				</tr>
				</table>
				</td>
			</tr>
