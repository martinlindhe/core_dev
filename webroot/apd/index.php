...
//settings:
$config['apd']['i'] = false;		//array_key_exists('i',$opt)
$config['apd']['T']	= false;		//array_key_exists('T', $opt)
$config['apd']['c']	= false;		//array_key_exists('c', $opt)
$config['apd']['t']	= false;		//array_key_exists('t', $opt)
$config['apd']['z']	= false;		//array_key_exists('z', $opt)
$config['apd']['Z']	= false;		//array_key_exists('Z', $opt)

$config['apd']['r']	= false;		//array_key_exists('r', $opt)
$config['apd']['R']	= false;		//array_key_exists('R', $opt)

$config['apd']['u']	= false;		//array_key_exists('u', $opt)
$config['apd']['U']	= false;		//array_key_exists('U', $opt)

$config['apd']['s']	= false;		//array_key_exists('s', $opt)
$config['apd']['S']	= false;		//array_key_exists('S', $opt)

$config['apd']['sort'] = 'by_time';
$config['apd']['O'] = 20;
...
$dataFile = 'E:\Devel\apd_dump\pprof.01772.1';
...
while($line = fgets($DATA)) {
    ...
    if( $token == '+') {
        ...
        if($config['apd']['i'] && $symbol_type[$index] == 1) {
            continue;
        }	
        $index_cur = $index;
        @$calls[$index_cur]++;
...
foreach (array_keys($symbol_hash) as $j) {
    if($config['apd']['i'] && $symbol_type[$j] == 1) {
        continue;
    }
    if ($l++ < $config['apd']['O']) {
