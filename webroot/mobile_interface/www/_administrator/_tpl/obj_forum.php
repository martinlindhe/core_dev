<?
	...
	if(!empty($_GET['del']) && is_numeric($_GET['del'])) {
		$sql->queryUpdate("DELETE FROM {$tab['thought']} WHERE main_id = '".secureINS($_GET['del'])."' LIMIT 1");
		...
		foreach($_POST as $key => $val) {
			if(strpos($key, 'status_id') !== false) {
				...
				if(isset($_POST['status_id:' . $kid])) {
					mysql_query("UPDATE {$tab['thought']} SET status_id = '".secureINS($_POST['status_id:' . $kid])."' WHERE main_id = '".secureINS($kid)."' LIMIT 1");
				}
			}
		}
		if($doall) mysql_query("UPDATE {$tab['thought']} SET status_id = '".secureINS($_POST['main_id:all'])."', view_id = '1' WHERE view_id = '0'");
	...
	if($view_gb) {
		...
		$list = $sql->query("SELECT a.*, u.id_id, u.u_alias, u.u_picvalid, u.u_picd, u.u_picid, d.u_alias as admin_alias FROM {$tab['thought']} a LEFT JOIN {$tab['user']} u ON a.logged_in = u.id_id LEFT JOIN {$tab['user']} d ON a.answer_id = d.id_id ORDER BY a.main_id DESC LIMIT {$paging['slimit']}, {$paging['limit']}", 0, 1);
	} else {
		$list = $sql->query("SELECT a.*, u.id_id, u.u_alias, u.u_picvalid, u.u_picd, u.u_picid, d.u_alias as admin_alias FROM {$tab['thought']} a LEFT JOIN {$tab['user']} u ON a.logged_in = u.id_id LEFT JOIN {$tab['user']} d ON a.answer_id = d.id_id WHERE a.view_id = '0' AND a.status_id = '0' ORDER BY a.main_id DESC", 0, 1);
	}
	$view_c = array(
'0' => $sql->queryResult("SELECT COUNT(*) as count FROM {$tab['thought']} WHERE view_id = '0' AND status_id = '0'"),
'1' => $sql->queryResult("SELECT COUNT(*) as count FROM {$tab['thought']}"));
...
<?=#(is_md5($row['logged_in']))?'<a href="user.php?t&id='.secureOUT($row['logged_in']).'"><span class="txt_big user">'.secureOUT($row['u_alias']).'</span></a>':'<span class="txt_bld txt_big">'.secureOUT($row['gb_name']).'</span>';?> - <b><?=$cities[$row['p_city']]?></b> - <em><?=niceDate($row['gb_date'])?></em> (#<?=$row['main_id']?>)<?=($row['status_id'] == '2')?'</span>':'';?></td>
			</tr>
			<tr class="<?=$class.$txt?>">
				<td>
				<table cellspacing="0">
				<tr>
<?=(!empty($row['logged_in']))?'					<td style="width: 57px; padding-right: 10px;">'.$user->getphoto($row['id_id'].$row['u_picid'].$row['u_picd'], $row['u_picvalid'], 1, 1).'</td>':'';?>
