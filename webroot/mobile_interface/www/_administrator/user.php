...
'NF' => ' på grund av: <b>Ej rakt framifrån.</b>');
	...
	if(!empty($_POST['doupd'])) {
		if(!empty($_POST['id']) && is_md5($_POST['id'])) {
			...
			$sql->queryUpdate("UPDATE {$t}user_info SET
			...
			foreach($_POST as $key => $val) {
				if(strpos($key, 'status:') !== false) {
					...
					if(isset($_POST['code:' . $kid]) && !empty($_POST['code:' . $kid])) {
						$sql->queryUpdate("UPDATE {$t}user_info SET reg_code = '".secureINS($_POST['code:' . $kid])."' WHERE id_id = '".secureINS($kid)."' LIMIT 1");
					}
					...
					if(isset($_POST['sendemail:' . $kid]) && !empty($_POST['sendemail:' . $kid])) {
						$inf = $sql->queryLine("SELECT i.reg_code, u.u_email FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE u.id_id = '".$kid."' LIMIT 1");
	...
	if(!empty($_GET['id']) && is_md5($_GET['id'])) {
		$row = $sql->queryLine("SELECT u.*, i.* FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE u.id_id = '".secureINS($_GET['id'])."' LIMIT 1", 1);
		if(!count($row)) {
			$change = false;
		} else {
			$change = true;
	...
	if(!$change) {
		if(!$status_id) {
			$list = $sql->query("SELECT a.id_id, i.reg_code, a.u_email, a.u_birth, a.u_regdate, i.u_cell FROM {$t}user a LEFT JOIN {$t}user_info i ON i.id_id = a.id_id WHERE a.status_id = 'F' ORDER BY a.u_regdate DESC");
		} elseif($status_id == 'N') {
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '1' AND view_id = '0' ORDER BY u_regdate DESC");
		} elseif($status_id == '2') {
			$sex = false;
			if(!empty($_GET['sex'])) {
				$sex = $_GET['sex'];
				$pics = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, u_picvalid, u_picid, u_picd FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '1' AND u_picvalid = '1' AND u_sex = '$sex' ORDER BY u_regdate DESC");
			} else
				$pics = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, u_picvalid, u_picid, u_picd FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '1' AND u_picvalid = '1' ORDER BY u_regdate DESC");
		} elseif($status_id == '3') {
			$sort = '';
			...
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, u_picvalid, u_picid, location_id, lastlog_date, lastonl_date, account_date FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '1' AND account_date > '".$user->timeout('30 MINUTES')."' ORDER BY ".(!$sort?'u_regdate DESC':($type == 'date'?'location_id '.$sort.', u_alias ASC':'lastlog_date '.$sort)));
		} elseif($status_id == '5') {
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, u_picvalid, u_picid, lastlog_date, u_regdate FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '2' ORDER BY lastlog_date DESC");
		} elseif($status_id == '10') {
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, level_enddate, level_pending, location_id FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '1' AND (level_pending = '1' OR level_pending = '0' AND level_id > 1) ORDER BY level_enddate ASC");
		} elseif($status_id == '6') {
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname, u_picvalid, u_picid FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE status_id = '3' ORDER BY lastlog_date DESC");
		} elseif($status_id == '4') {
			$list = false;
			$pics = false;
		} else {
			$list = $sql->query("SELECT u.id_id, status_id, level_id, u_alias, u_email, u_fname, u_sname FROM {$t}user u LEFT JOIN {$t}user_info i ON i.id_id = u.id_id WHERE view_id = '1' AND status_id = '1' ORDER BY u_regdate DESC");
		}
	} else $list = '';
