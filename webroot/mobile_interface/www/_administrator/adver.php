...
<?
	...
	foreach($lpsdl as $row) {
		...
		echo '<tr class="bg_gray">
			<td style="width: 80px; padding: 0 0 0 4px;"><nobr><img src="./_img/status_'.(($row[7] == '1')?'green':'none').'.gif" style="margin: 4px 1px 0 0;" id="1:'.$row[3].'" onclick="changeStatus(\'status\', this.id);"><img src="./_img/status_'.(($row[7] == '2')?'red':'none').'.gif" style="margin: 4px 0 0 1px;" id="2:'.$row[3].'" onclick="changeStatus(\'status\', this.id);"> <input type="text" name="order_id:'.$row[3].'" value="'.$row[8].':'.$row[9].'" disabled style="width: 24px; padding: 0; margin-bottom: 4px; line-height: 9px; height: 11px; size: 10px;" onfocus="this.select();" class="inp_nrm"></nobr></td>
			<td class="cur" onclick="document.location.href = \'adver.php?status='.$status_id.'&id='.$row[3].'\';"'.(($gotpic)?' onmouseover="document.getElementById(\'tr:'.$row[3].'\').style.display = \'\';" onmouseout="document.getElementById(\'tr:'.$row[3].'\').style.display = \'none\';"':'').' style="width: 350px; padding: 4px;"><nobr>'.$row[13].' <b>'.$row[4].'</b> (#'.$row[3].')</nobr></td>
			<td class="cur" onclick="document.location.href = \'adver.php?status='.$status_id.'&id='.$row[3].'\';" style="padding: 4px;">'.$row[2].'</td>
			<td class="cur" onclick="document.location.href = \'adver.php?status='.$status_id.'&id='.$row[3].'\';" style="padding: 4px;"><nobr>[ '.$row[10].' visningar | '.$row[11].' ('.$row[12].') besök ]</nobr></td>
			<td class="cur" onclick="document.location.href = \'adver.php?status='.$status_id.'&id='.$row[3].'\';" style="padding: 4px;"><nobr>'.((strtotime($row[5]) <= time() && strtotime($row[6]) >= time())?plainDate($row[5], 0).' '.plainDate($row[6], 0):'<strike>'.plainDate($row[5], 0).' '.plainDate($row[6], 0).'</strike>').'</nobr></td>
			<td class="cur" style="padding: 4px;" align="right"><nobr><a href="adver.php?status='.$status_id.'&id='.$row[3].'">ÄNDRA</a> | <a href="adver.php?del='.$row[3].'" onclick="return confirm(\'Säker ?\');">RADERA</a></nobr></td>
		</tr>';
