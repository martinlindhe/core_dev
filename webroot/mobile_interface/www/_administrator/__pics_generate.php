...
if(!empty($_GET['do_gen'])) {
		...
		header("Location: pics_generate.php?id=".$row['main_id']."&dir=".$file."&mf=".$movefiles."&gm=".$got_movie."&owner=".((!empty($p_owner))?$p_owner:$p_own));
		exit;
		...
		if(count($img_dir['files']) > 0) { foreach($img_dir['files'] as $val) {
			...
			case 2:
				...
				$errors = 0;
		      		$errors += doThumb($val, $gen_dir.$file_id.$p_name, $upl_full[0], $upl_full[1]);
				$errors += doThumb($gen_dir.$file_id.$p_name, $gen_dir.$file_id.'-thumb'.$p_name, $upl_thumb[0], $upl_thumb[1], 90);
				$errors += doWM($gen_dir.$file_id.$p_name, $gen_dir.$file_id.$p_name, 93, $row['p_exclusive']);

				rename($val, $gen_dir.$file_id.'-full1537'.$p_name);
				#if($movefiles) {
					rename($gen_dir.$file_id.$p_name, ADMIN_IMAGE_DIR.$row['main_id'].'/'.$file_id.$p_name);
					rename($gen_dir.$file_id.'-full1537'.$p_name, ADMIN_IMAGE_DIR.$row['main_id'].'/'.$file_id.'-full1537'.$p_name);
					rename($gen_dir.$file_id.'-thumb'.$p_name, ADMIN_IMAGE_DIR.$row['main_id'].'/'.$file_id.'-thumb'.$p_name);
				#}
				mysql_query("INSERT INTO $pic_tab SET status_id = '".(($movefiles)?'0':'1')."', topic_id = '".secureINS($row['main_id'])."', id = '$file_id', p_pic = '".substr($p_name, 1)."', p_date = NOW(), owner_id = '".secureINS(((!empty($p_owner))?$p_owner:$p_own))."'");
	...
	$mv = 'pics_generate.php?id='.$row['main_id'].'&dir='.$dir.'&mf='.$movefiles.'&gm='.$got_movie.'&owner='.$p_owner;
	$time = '1000';
