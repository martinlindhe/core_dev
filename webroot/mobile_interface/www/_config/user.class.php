...
class user {
	...
	function getimg($arr, $valid = 1, $big = 0, $extra = '') {
		...
		return (!$id?'<a>':'<a href="'.l('user', 'view', $id).'"'.(!empty($extra['text'])?' title="'.secureOUT($extra['text']).'"':'').(!empty($extra['toparent'])?' target="_blank" onclick="if(window.opener) { window.opener.location.href = this.href; window.opener.focus(); return false; }"':'').'>').'<img'.(!empty($extra['text'])?' alt="'.secureOUT($extra['text']).'"':'').' src="'.($valid?UPLA.'images/'.$pd.'/'.$id.$arr.(!$big?'_2':'').'.jpg':'/_objects/u_noimg'.$sex.(!$big?'_2':'').'.gif').'" '.($big?'class="bbrd" style="width: 150px; height: 150px;"':'class="brd" style="width: 50px; height: 50px;"').' /></a>';
	}
	...
	function getstring($arr, $suffix = '', $extra = '') {
		...
		elseif(empty($arr['u_alias']))
			...
			$own = ($arr['id_id'] == $this->id?true:false);
		  	//$mail_add = (!empty($extra['top']))?'<a href="javascript:makeSmall(\'/user/mailwrite/'.$arr['id_id'].'\'); " title="Skriv mail"><img alt="" src="'.OBJ.'mail_write.gif" style="margin: 0 0 -2px 3px;" /></a>':'';
			...
			return (empty($extra['nolink'])?'<a href="'.l('user', 'view', $arr['id_id'.$suffix]).'"':'<span').' class="bld user"><span class="'.($this->isOnline($arr['account_date'.$suffix])?'on':'off').'"'.(!isset($extra['noimg'])?' onmouseover="launchHover(event, \''.$arr['id_id'].'\');" onmouseout="clearHover();"':'').'>'.secureOUT($arr['u_alias'.$suffix]).'</span>'.(empty($extra['nosex'])?' <img alt="'.$sex_name[$arr['u_sex'.$suffix]].'" src="/_objects/icon_'.$arr['u_sex'.$suffix].'1.gif" />':'').(empty($extra['noage'])?'<span class="sex'.$arr['u_sex'.$suffix].'">'.$this->doage($arr['u_birth'.$suffix]).'</span>':'').(empty($extra['nolink'])?'</a>':'</span>').(!empty($extra['top']) && !$own?'<nobr><a href="javascript:makeGb(\''.$arr['id_id'].'\''.(($extra && @$extra['gb'])?', \'&a='.$extra['gb'].'\'':'').');" title="Skriv GB-inlägg"><img alt="Skriv GB-inlägg" src="'.OBJ.'guestbook_write.gif" style="margin: 0 0 -2px 3px;" /></a><a href="javascript:makeChat(\''.$arr['id_id'].'\');" title="Öppna privatchat"><img alt="Öppna privatchat" src="'.OBJ.'chat_write.gif" style="margin: 0 0 -2px 3px;" /></a>'.$mail_add.'</nobr>':'');
		}
