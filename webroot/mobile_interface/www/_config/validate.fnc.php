<?
function doMail($to = '', $title = '', $text = '', $special = 0) {
	ini_set('sendmail_from', 'info@'.URL);
	ini_set("SMTP", SMTP_SERVER); #193.13.74.200"); #195.26.11.113");
	$border = md5(time());
	#$mail_boundary = "x{$border_random}x";
	if(!$special) $headers  = "Return-Path: <info@".URL.">\nFrom: \"".NAME_URL."\" <info@".URL.">\nReply-To: <info@".URL.">\nMIME-Version: 1.0\nContent-Type: text/plain; charset=ISO-8859-1\n"; 
	else $headers  = "Return-Path: <admin@".URL.">\nFrom: \"ADMIN - ".NAME_TITLE."\" <admin@".URL.">\nReply-To: <admin@".URL.">\nMIME-Version: 1.0\nContent-Type: text/html; charset=ISO-8859-1\n"; 
	if(mail($to, $title, $text, $headers)) return true; else return false;
}
...
function valiField($str, $type='normal', $val = '0') {
	if($type == 'normal')
		$pattern = "^[A-ZÁÀÉÈËÊÍÌÑÛÜÚÙÝÅÄÖ´`a-záàéèëêóòíìñûüúùýÿåäöÅÄÖ .\/-]+$";
	elseif($type == 'user')
	...
	elseif($type == 'street')
		$pattern = "^[A-Z0-9ÁÀÉÈËÊÍÌÛÜÚÙÝÅÄÖ´`a-záàéèëêíìóòûüúùýÿåäöÅÄÖ ,.\/:-]+$";
	elseif($type == 'postnr')
		$pattern = "^[0-9]{5}$";
	elseif($type == 'cell')
		$pattern = "^(07)[0-9]{8}$";
