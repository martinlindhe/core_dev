...
function makeMenu($sel, $arr) {
	...
	foreach($arr as $key => $val) {
		...
		echo '<a href="'.$val[0].'"><img style="margin-right: 1px;" src="/_objects/_heads/btn_'.$val[1].((strtolower($key) == strtolower($sel))?'_active':'').'.png" /></a>';
	}
}
function flash($s) {
...
function l($type = 'main', $action = 'index', $id = '', $key = '') {
...
function splashIACT($msg, $topic, $tc = 1, $class = '') {
	errorACT($msg, '', 'popup', '', 3000, $topic, $tc, $class);
}
function errorIACT($msg, $topic, $tc = 1, $class = '') {
	errorACT($msg, '', 'main', '', 3000, $topic, $tc, $class);
}
function errorTACT($msg, $url, $time) {
	errorACT($msg, $url, 'main', '', $time);
}
function errorACT($msg, $url = '', $type = 'main', $parent = '', $time = 3000, $topic = '', $tc = 1, $class = '') {
	eval(GLOBAL_STRING);
	if($type == 'main') {
		require(DESIGN.'error.php');
		if(!empty($url)) {
...
function popupLACT($msg, $cnt = false) {
	errorACT($msg, 1, 'popup', '', 3000, $cnt);
}
function popupACT($msg, $url = '', $parent = '', $time = 3000) {
	errorACT($msg, $url, 'popup', $parent, $time);
}
function splashLACT($msg, $cnt = false) {
	errorACT($msg, $url, 'splash', '', 3000, $cnt);	
}
function splashACT($msg, $url = '', $time = 3000) {
...
function dopaging($paging, $url, $anchor = '', $width = 'med', $text = '&nbsp;', $vice = 1) {
	...
	if($width == 'med' || $width == 'medmin' || $width == 'big' || $width == 'medbig') echo '<table cellspacing="0" style="font-family: Verdana; font-size: 10px; width: '.($width == 'medmin'?'510':($width == 'big'?'935':($width == 'medbig'?'915':'530'))).'px;"><tr>
	...
	if($width == 'big' || $width == 'medbig' || $width == 'med' || $width == 'medmin') echo '&nbsp;&nbsp;'.($paging['p'] < $stop?'<a href="'.$url.($paging['p']+1).$anchor.'" class="bld">'.((!$vice)?'bakåt':'framåt').' »</a>':'&nbsp;').'</td>
	</tr></table>';
}
...
function errorACT($txt, $link = '', $time = '3000', $topic = '') {
...
function popupACT($msg, $mv = '', $time = '3000', $parent = '') {
	global $NAME_TITLE;
	require("./_tpl/notice_popup.php");
	exit;
}

function splashACT($msg, $link = '', $time = '3000') {
	global $NAME_TITLE;
