...
<?
	foreach($result as $pic) {
		echo '<option value="'.PHOTO_DIR.$pic[2].'/'.$pic[0].(($pic[3])?'_'.$pic[4]:'').'.'.$pic[5].'">'.secureOUT('#'.$pic[0].' - '.$pic[6]).' '.(($pic[3])?'<b>[privat]</b>':'').'</option>';
	}
?>
...
function replaceALIAS(id) {
	...
	str = '<IMG src="' + id + '">';
	str2 = '<img src="' + id + '">';
	_d.getElementById(TC_name + '_html').value = _d.getElementById(TC_name + '_html').value.replace(str, '<a target="commain" href="./user.php?id=' + n_id + '" title="' + alias + '"><img alt="' + alias + '" src="' + id + '" alt="" /></a>');
	_d.getElementById(TC_name + '_html').value = _d.getElementById(TC_name + '_html').value.replace(str2, '<a target="commain" href="./user.php?id=' + n_id + '" title="' + alias + '"><img alt="' + alias + '" src="' + id + '" alt="" /></a>');
