...
<tr>
	...
	<td class="spac rgt pdg_tt"><a href="'.l('user', 'relations', $row['id_id'], '0').'&a='.$row['main_id'].'"><img src="'.OBJ.'icon_yes.gif" style="margin-bottom: -4px;" title="Godkänn" /></a> - <a href="'.l('user', 'relations', $row['id_id'], '0').'&d='.$row['id_id'].'" onclick="return confirm(\'Säker ?\');"><img src="'.OBJ.'icon_no.gif" style="margin-bottom: -4px;" title="Neka" /></a></td>
</tr>
';
...
#		$gotpic = ($row['u_picvalid'] == '1')?true:false;
		...
		echo '
			<tr>
				...
				<td class="spac pdg rgt">'.nicedate($row['sent_date']).'</td>
				<td class="spac rgt pdg_tt"><a href="'.l('user', 'relations', $l['id_id'], '0').'&c_w='.$row['main_id'].'#w'.$row['main_id'].'"><img src="'.OBJ.'icon_change.gif" title="Ändra" style="margin-bottom: -4px;" /></a> - <a href="'.l('user', 'relations', $row['id_id'], '0').'&d='.$row['id_id'].'" onclick="return confirm(\'Säker ?\');"><img src="'.OBJ.'icon_no.gif" style="margin-bottom: -4px;" title="Sluta vänta" /></a></td>
			</tr>';
		if($c == $row['main_id']) {
?>
<tr>
