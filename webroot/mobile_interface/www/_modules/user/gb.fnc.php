...
function gbList($user_id, $_start = 0, $_end = 0) {
	global $sql, $t;
	$q = "SELECT ".CH." gb.*, u.id_id, u.u_alias, u.u_picid, u.u_picd, u.u_picvalid, u.account_date, u.status_id, u.u_sex, u.u_birth, u.level_id FROM {$t}usergb gb LEFT JOIN {$t}user u ON u.id_id = gb.sender_id AND u.status_id = '1' WHERE gb.user_id = '".secureINS($user_id)."' AND gb.status_id = '1' ORDER BY gb.main_id DESC";
	if ($_start || $_end) $q .= ' LIMIT '.$_start.','.$_end;
	return $sql->query($q, 0, 1);
}
// to return a specific msg by its id. will return false if the user or the message is deleted.
function getMsgById($msg_id) {
	//INNER JOIN is to only view msg where the sender-user still exists.
	return $sql->queryLine("SELECT gb.*, u.id_id, u.u_alias, u.u_picid, u.u_picd, u.u_picvalid, u.account_date, u.status_id, u.u_sex, u.u_birth, u.level_id FROM {$t}usergb gb INNER JOIN {$t}user ON u.id_id = gb.sender_id AND u.status_id = '1' WHERE gb.main_id = '".secureINS($msg_id)."' AND gb.status_id = '1'", 1);
}
...
function gbHistory($first_user, $second_user, $_start = 0, $_end = 0) {
	global $sql, $t;
	sort($or);
	$q = "SELECT ".CH." gb.*, u.id_id, u.u_alias, u.u_picid, u.u_picd, u.u_picvalid, u.account_date, u.status_id, u.u_sex, u.u_birth, u.level_id FROM {$t}usergbhistory h INNER JOIN {$t}usergb gb ON gb.main_id = h.msg_id AND gb.status_id = '1' LEFT JOIN {$t}user u ON u.id_id = gb.sender_id AND u.status_id = '1' WHERE h.users_id = '".implode('-', $or)."' ORDER BY h.main_id DESC";
	if ($_start || $_end) $q .= ' LIMIT '.$_start.','.$_end;
	return $sql->query($q, 0, 1);
}
