<?
    ...
    $res = $sql->queryUpdate("UPDATE {$t}userblogcmt SET status_id=0 WHERE main_id = '".secureINS($_GET['del_msg'])."'");
    $res = $sql->queryUpdate("UPDATE {$t}userblog SET blog_cmts=(blog_cmts-1) WHERE main_id = '".secureINS($_GET['key'])."'");
    reloadACT(l('user', 'blog', $id, $_GET['key']));
	}*/
	if(!empty($_GET['d'])) {
	...
	#} else {
	#	$res = $sql->query("SELECT main_id, blog_title, blog_cmts, blog_date, hidden_id, blog_visit FROM {$t}userblog WHERE blog_idx = '".$date."' AND user_id = '".$s['id_id']."' AND status_id = '1' ORDER BY main_id DESC", 0, 1);
	#	$paging['co'] = $sql->queryResult("SELECT COUNT(*) as count FROM {$t}userblog WHERE blog_idx = '".$date."' AND user_id = '".$s['id_id']."' AND status_id = '1'");
	#}
	$page = 'blog';
...
<tr>
	...
	<td onclick="'.$url.'" class="cur pdg spac rgt"><nobr>'.nicedate($row['blog_date']).'</nobr></td>
	<td class="spac rgt pdg_tt">'.(($own || $isAdmin)?'<nobr><a href="javascript:makeBlog(\''.$s['id_id'].'\', \''.$row['main_id'].'\');"><img src="'.OBJ.'icon_change.gif" title="Ändra" style="margin-bottom: -4px;" /></a> - <a class="cur" onclick="if(confirm(\'Säker ?\')) goLoc(\''.l('user', 'blog', $s['id_id'], '0').'&d='.$row['main_id'].'\');"><img src="'.OBJ.'icon_del.gif" title="Radera" style="margin-bottom: -4px;" /></a>':'').'</nobr></td>
</tr>';
		}
}
	} else {
