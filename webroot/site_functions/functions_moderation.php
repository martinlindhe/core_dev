<?
	/* functions_moderation.php - Funktioner för moderering
		...
		this module uses functions_comments.php to store comments to the moderation queue
	...
	*/
	...
	function isObjectionable(&$db, $text)
	{
		/* Removes non-letters */
		$text = str_replace("\n", " ", $text);
		...
		for ($i=0; $i<count($list); $i++) {
			...
			$sql = 'SELECT wordText,wordMatch FROM tblStopwords WHERE wordType='.STOPWORD_OBJECTIONABLE;
	...
	function addToModerationQueue(&$db, $itemId, $queueType)
	{
		...
		$sql = 'INSERT INTO tblModerationQueue SET queueType='.$queueType.',itemId='.$itemId.',timestamp='.time();
