...
*/
	...
	function getInfoFieldHistory(&$db, $fieldId)
	{
	...
	function formatInfoField(&$db, &$data, $fieldName)
	{
		global $config;
		
		$text = dbStripSlashes($data['fieldText']);
	...
	function getInfoField(&$db, $fieldName)
	{
		...
		if ($_SESSION['isSuperAdmin'] && !isset($_GET['hideinfopanel']))
		{
			...
			{
				$info = formatInfoField($db, $data, $fieldName);
			}
			...
			$html = $js.
								'<div class="infofield">'.
									'<div class="infofield_head"><ul>'.
										...
										'<li>'.($current_tab=='edit'?'<strong>':'').		'<a href="'.AddToURL('infoedit', $fieldName, $_SERVER['PHP_SELF']).'">'.$config['text']['link_edit'].'</a>'.	($current_tab=='edit'?'</strong>':'').'</li>'.
