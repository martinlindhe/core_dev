<?
	...
	if (!empty($_POST['lyricid']) && is_numeric($_POST['lyricid']))
	{
		...
		if (!linkLyric($record_id, $track, $lyric_id, $coverband_id))
		{
	...
	if ($coverband_id == 0) {
		...
		echo '2. Select the band who did the orginal song that <b>'.getBandName($coverband_id).'</b> covers.<br/>';
		echo '(Adding cover to track <b>'.$track.'</b> on <b>'.getRecordName($record_id).'</b>).<br/><br/>';
		...
		echo '3. Select the original song by <b>'.getBandName($band_id).'</b> that <b>'.getBandName($coverband_id).'</b> covers from the dropdown below.<br/>';
		echo '(Adding cover to track <b>'.$track.'</b> on <b>'.getRecordName($record_id).'</b>).<br/><br/>';
