<?
	...
	$cmt_arr = $db->getOneItem("SELECT COUNT(*) FROM s_userblog");

	$paging = paging(@$_GET['p'], 20);
	$list = $db->getArray("SELECT a.*, u.id_id, u.u_alias, u.u_picd, u.u_picvalid, u.u_picid, u_sex FROM s_userblog a LEFT JOIN s_user u ON u.id_id = a.user_id AND u.status_id = '1' ORDER BY a.main_id DESC LIMIT {$paging['slimit']}, {$paging['limit']}");
?>
...
?>
 ...
 - <em>inl&auml;gg skrivet <?=niceDate($r['blog_date'])?></em> (#<?=$r['main_id']?>)</span></td></tr>
				<tr>
					<td style="width: 100%;"><?='<table cellspacing="0"><tr><td>'.getadminimg($r['id_id']).'</td><td style="padding-left: 5px;"><div style="width: 470px; overflow: hidden;"><b>'.secureOUT($r['blog_title']).'</b><br /><br />'.formatText($r['blog_cmt']).'</div></td></tr></table>';?></td>
				</tr>
				</table>
				</td>
			</tr>
