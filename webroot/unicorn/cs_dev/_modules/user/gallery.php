<?
	...
	$res = $sql->query($q, 0, 1);
	$paging['co'] = $sql->queryResult("SELECT COUNT(*) as count FROM s_userphoto WHERE user_id = '".$s['id_id']."' AND status_id = '1'");
	...
	*/
	...
	$gall_res = $sql->query("SELECT * FROM s_userphoto WHERE user_id = '".$s['id_id']."' AND status_id = '1' AND hidden_id = '0' ORDER BY main_id DESC", 0, 1);
	$gallx_res = $sql->query("SELECT * FROM s_userphoto WHERE user_id = '".$s['id_id']."' AND status_id = '1' AND hidden_id = '1' ORDER BY main_id DESC", 0, 1);
...
/* <script type="text/javascript">var first = '<?=$first?>'; ext = '<?=$ext?>';</script> */
		$paging = paging(1, 20);
		$paging['co'] = $sql->queryResult("SELECT COUNT(*) as count FROM s_userphoto WHERE user_id = '".$s['id_id']."' AND status_id = '1' AND hidden_id = '0' LIMIT 1");
		$name = 'galleri';
		$all = true;
		$res = $gall_res;
...
<?
	if ( $s['id_id'] != $l['id_id'] && !getGallXStatus($s['id_id']) && !$user->vip_check(10)) {
		echo 'Denna medlem har inte tillåtit dig att se dennes galleri x-bilder.';
	} else {
		$paging = paging(1, 20);
		$paging['co'] = $sql->queryResult("SELECT COUNT(*) as count FROM s_userphoto WHERE user_id = '".$s['id_id']."' AND status_id = '1' AND hidden_id = '1'");
		$name = 'galleri x';
		$all = $allowed;
		$res = $gallx_res;
		include('gallerylist.php');
