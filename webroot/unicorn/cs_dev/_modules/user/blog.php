<?
	...
	$res = $sql->query("SELECT main_id, blog_title, blog_cmts, blog_date, hidden_id, blog_visit FROM s_userblog WHERE user_id = '".$s['id_id']."' AND status_id = '1' ORDER BY main_id DESC LIMIT {$paging['slimit']}, {$paging['limit']}", 0, 1);
	$paging['co'] = $sql->queryResult("SELECT COUNT(*) as count FROM s_userblog WHERE user_id = '".$s['id_id']."' AND status_id = '1'");
	$page = 'blog';
