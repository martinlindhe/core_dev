<?
	...
	if(strpos($ua, 'msie') == true && strpos($ua, 'msie 7') == false)
		...
		$ua = 0;
	$actuals = date_diff(date("Y-m-d"), $user->getline('u_picdate', $user->id));
	$actuals = $actuals['days'];
	if(array_key_exists($_SESSION['data']['level_id'], $length)) {
		$actual = $length[ $_SESSION['data']['level_id'] ];
		$actual -= $actuals;
	} else $actual = 0;
	if($actual < 0) $actual = 0;
	$isG = $user->level($_SESSION['data']['level_id'], 6);
	$ua = strtolower($_SERVER['HTTP_USER_AGENT']);
...
<div id="mainContent">
	...
	<? makeButton(true, "document.location='settings_img.php'", 'icon_settings.png', 'bild'); ?>
