	...
	for ($i=0; $i<count($list); $i++) {
		...
		echo '<table width="100%" cellpadding=0 cellspacing=0 border=0>';
			echo '<tr><td colspan=2 class="'.$classname.'">';
				...
				echo 'Comment: '.$comment.'<br><img src="gfx/c.gif" width=1 height=10>';
			echo '</td></tr>';
			...
			echo '<tr><td class="'.$classname.'">';
				...
				echo ' from <a href="admin_ip.php?ip='.$ip_v4.'">'.$ip_v4;
				echo '</a>';

				echo ' <img src="../flags/'.GeoIP_ci_to_CountryShort($list[$i]['ci']).'.png" align="top" width=16 height=11 title="'.GeoIP_ci_to_Country($list[$i]['ci'])."> ";
				echo makeTimePeriodShort( time()-$list[$i]['timeAdded'] ).' ago';
			echo '</td>';
		...
		echo '</table>';
		echo '<img src="gfx/c.gif" width=1 height=10><br>';
	}
?>
