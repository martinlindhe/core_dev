<?
	...
	$sql = $db->getArray("SELECT * FROM s_poll ORDER BY poll_month DESC");
...
<?
		...
		$poll = $db->getOneRow("SELECT * FROM s_poll WHERE poll_month = '".date("Y-m-00")."' LIMIT 1");
...
<?
	...
	if(!empty($_GET['view']) && $active != $_GET['view']) {
		...
		$poll = $db->getOneRow("SELECT * FROM s_poll WHERE main_id = '{$_GET['view']}' LIMIT 1");
...
<?
						if (!empty($_GET['view']) && $_GET['view'] == $poll['main_id'] && !empty($_GET['answer']) && $_GET['answer'] == $ans) {
							$iql = $db->getArray("SELECT main_id, sess_ip, date_cnt, is_enabled FROM s_pollvisit WHERE category_id = '{$poll['main_id']}' AND unique_id = '$ans' ORDER BY is_enabled, date_cnt DESC");
							if (!empty($iql)) {
								echo '<tr><td><table cellspacing="0" style="margin-top: 5px;">';
								foreach ($iql as $irow) {
									$disabled = (!$irow['is_enabled'])?'[<b>SUBTRAHERAD</b>] ':'';
									echo '<tr><td style="padding-right: 20px;">'.$disabled.'<span>' . $irow['sess_ip'].'</td><td style="padding-right: 10px;">'.niceDate($irow['date_cnt']).'</span></td><td><a href="poll.php?view='.$poll['main_id'].'&answer='.$_GET['answer'].'&'.(($irow['is_enabled'])?'deny='.$irow['main_id'].'">SUBTRAHERA':'acc='.$irow['main_id'].'">ADDERA').'</a></td></tr>';
								}
								echo '</table></td></tr>';
							}
						}
